...
class Node {
	...
	public function __construct(array $to_hydrate = null){
		$this->addProperty('Integer', 'sd_node', 'id');
		$this->addProperty('Integer', 'sd_node', 'parent_id');
		$this->addProperty('Node', null, 'parent');
		$this->addProperty('Integer', 'sd_node', 'object_id');
		$this->addProperty('Integer', 'sd_node', 'object_type_id');
		$this->addProperty('Boolean', 'sd_node', 'inherit_permissions');
		$this->addProperty('Text', 'sd_version', 'lang');
		$this->addProperty('Integer', 'sd_version', 'revision_number');
		$this->addProperty('Integer', 'sd_version', 'object_id');
		$this->addProperty('Date', 'sd_version', 'revision_date');
		$this->addProperty('Integer', 'sd_object', 'created_by_id');
		$this->addProperty('Node', null, 'created_by');
		$this->addProperty('Date', 'sd_object', 'created_at');
		$this->addProperty('Integer', 'sd_object', 'modified_by_id');
		$this->addProperty('Node', null, 'modified_by');
		$this->addProperty('Date', 'sd_object', 'created_at');
		$this->addCondition('sd_node.id = :PK');
		$this->addCondition('sd_version.node_id = sd_node.id');
		$this->addCondition('sd_version.object_id = sd_object.id');
		if($to_hydrate){
			$this->hydrate($to_hydrate);
		}
	 ...
	 */
	public static function retrieveById($id, $lang = null, $revision = null){
		$id = (int)$id;
		$node = new self();
		$stmt = $node->getSelectStatement();
		...
		if(!$row){
			throw new \Sidus\Exceptions\NodeNotFoundException("Unable to retrieve node number {$id}");
		}
		$node->hydrate($row);
		return $node;
	}
	...
	public function __get($var){
		$var = strtolower($var);
		$getter_name = 'get'.\Sidus\Core\Utils::camelize($var);
		...
		if(array_key_exists($var, $this->properties)){
			...
			if($property->canRead($this->auths)){
				...
				throw new \Sidus\Exceptions\PermissionException("Trying to get property: {$var} from ".__CLASS__);
			}
		}
		throw new \Sidus\Exceptions\InexistingPropertyException("Trying to get inexisting property: {$var} from ".__CLASS__);
	}
	...
	public function __set($var, $value){
		$var = strtolower($var);
		$setter_name = 'set'.\Sidus\Core\Utils::Camelize($var);
	...
	public function __call($name, $arguments){
		...
		if(substr($name, 0, 3) == 'get'){
			return $this->__get(\Sidus\Core\Utils::unCamelize(substr($name, 3)));
		}
		if(substr($name, 0, 3) == 'set'){
			$this->__set(\Sidus\Core\Utils::unCamelize(substr($name, 3)));
			return $this;
		}
	...
	protected function hydrate(array $line = null){
		if($line){
			...
			foreach($properties as $key => $property){
				...
				$full_column_name = $property->getFullColumnName('_');
				if(array_key_exists($full_column_name, $line) && $property->check($line[$full_column_name])){
			...
			foreach($properties as $property){
				...
				$missing[] = $property->getFullColumnName('_');
			}
			if(count($missing) == 0){
				return true;
			}
			throw new \Sidus\Exceptions\HydratationException("Hydratation failed : Missing properties are ".implode(', ', $missing));
		}
...
//		$this->hydrate($statement->fetch(PDO::FETCH_ASSOC));
	...
	/**
	 * Return a PDO Statement for select
	 * @return PDOStatement
	 */
	protected function getSelectStatement(){
		...
		foreach($this->properties as $property){
			...
			$columns[] = $property->getFullColumnName().' AS '.$property->getFullColumnName('_');
		}
		$tables = implode(', ', $tables);
		$columns = implode(', ', $columns);
		$conditions = implode(' AND ', $this->conditions);
		$query = "SELECT {$columns} FROM {$tables} WHERE {$conditions} ORDER BY sd_version.revision_number DESC LIMIT 1";
		echo $query;
	...
	protected function addProperty($class_name, $table_name, $column_name, $model_name = null){
		if(substr($class_name, 0, 1) != '\\'){
			$class_name = '\\Sidus\\Properties\\'.$class_name;
		}
		$property = new $class_name($this, $table_name, $column_name, $model_name);
		if(!($property instanceof \Sidus\Properties\PropertyInterface)){
			throw new \Sidus\Exceptions\InexistingPropertyException("Wrong class_name, must be a PropertyInterface");
		}
	...
	protected function addCondition($condition){
		if(!$condition || array_search($condition, $this->conditions)){
			return false;
		}
	...
	protected static function debugStmt(\PDOStatement $stmt){
...
//	}
	...
	public static function get_add_form($action = '', $method = 'post'){
		require_once SIDUS_DIR.'includes/interface/class.ui_form.php';
		require_once SIDUS_DIR.'includes/interface/class.Input.php';
		require_once SIDUS_DIR.'includes/interface/class.ui_textarea.php';
	...
	protected function initPermissionsForm(){
		...
		//Basic HTML inputs for most of the fields
		require_once SIDUS_DIR.'includes/interface/class.Input.php';
		require_once SIDUS_DIR.'includes/interface/class.Checkbox.php';
		require_once SIDUS_DIR.'includes/interface/class.Select.php';
