...
class Core {
	 ...
	 */
	public static function initDb($config_path = null, $config_key = 'database'){
		if(!$config_path){
			$config_path = PROJECT_DIR.'config/database.ini';
		}
		if(!file_exists($config_path)){//Check the existence of the configuration file
			throw new Exceptions\ConfigurationException("No config file at {$config_path}");
		}
		$db_conf = parse_ini_file($config_path, $config_key);
		$db_conf = $db_conf[$config_key];
		if(!isset($db_conf['dsn'])){
			throw new Exceptions\ConfigurationException("Missing dsn in {$config_path}");
		}
		$dsn = $db_conf['dsn'];
		$username = isset($db_conf['username'])?$db_conf['username']:null;
		$password = isset($db_conf['password'])?$db_conf['password']:null;
		$driver_options = isset($db_conf['driver_options'])?$db_conf['driver_options']:null;
		$db = new Database($dsn, $username, $password, $driver_options);
	}
	...
	private function loadConfig(){
		...
		if(!is_writable(SIDUS_DIR.'secure')){//TODO : Drop a slightly more elegant error
			echo '<div style="position:absolute;background:red;width:60%;left:20%;padding:5px;top:9px;"><b>WARNING !</b> Secure directory is not writable:<br/>'.SIDUS_DIR.'secure/</div>';
		}
	}
	...
	private function loadDate(){
		require_once SIDUS_DIR.'includes/class.sys_date.php';
	...
	private function loadError(){
		require_once SIDUS_DIR.'includes/class.sys_error.php';
	...
	private function loadSession(){
		require_once SIDUS_DIR.'includes/class.sys_session.php';
	...
	private function loadUser(){
		require_once SIDUS_DIR.'includes/nodes/class.node_user.php';
	...
	private function loadPlugins(){
		$plugins_folders = SIDUS_DIR.'plugins/';
	...
	public final function getClassPath($type){
		...
		return SIDUS_DIR.$this->nodes_types[$type]['class_path'];
	}
	...
	public function getThumb($type_name = 'generic'){
		...
		if(file_exists(SIDUS_DIR.$iconsd.$type_name.'.png')){
			$filename = $type_name.'.png';
		}
		if(file_exists(PROJECT_DIR.$iconsd.$type_name.'.png')){
			$filename = $type_name.'.png';
	...
	public function localize($string){
		$file = SIDUS_DIR.'includes/localization/general.'.$this->user()->getData('lang').'.php';
		if(file_exists($file)){
			include $file;
