...
use SICLA\AraBundle\Entity\AffectationGroupe;
use SICLA\AraBundle\Form\AffectationGroupeType;
use SICLA\AraBundle\Form\AffectationGroupeAddFamilleType;
use Doctrine\Common\Collections\ArrayCollection;

class AffectationGroupeController extends CommonController
{
	public function showAction($loaded_objects) {
		return $this->render('SICLAAraBundle:AffectationsGroupes:show.html.twig', $loaded_objects);
	}
	
	public function editAction($version, $object, $loaded_objects, Request $request) {
		$form = $this->createForm(new AffectationGroupeType(), $object);
		$em = $this->getDoctrine()->getEntityManager();
		if ($request->isMethod('POST')) {
			 $form->bind($request);
				$em->flush();
								$this->setFlash('success', 'Sélection de la famille');
								return $this->redirect($this->generateUrl('sicla_ara_addFamille_groupe', 
		...
		return $this->render('SICLAAraBundle:AffectationsGroupes:edit.html.twig', $loaded_objects);
	}
	...
	public function addFamilleAction(Request $request, $node_id, $lang = null) {
		...
		$form = $this->createForm(new AffectationGroupeAddFamilleType(), $this->loaded_objects['object']);
		$em = $this->getDoctrine()->getEntityManager();
		...
		$loaded_objects['form'] = $form->createView();
		return $this->render('SICLAAraBundle:AffectationsGroupes:addFamille.html.twig',$loaded_objects);
	}
	...
	public function addAction($node, $lang, $type) {
		...
		$new_object = new AffectationGroupe();
